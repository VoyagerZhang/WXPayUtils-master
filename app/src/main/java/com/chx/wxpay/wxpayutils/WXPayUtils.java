    ...
    public void toWXPayNotSign(Context context, String appid) {
        ...
        Runnable payRunnable = new Runnable() {  //这里注意要放在子线程
            ...
            public void run() {
                ...
                request.appId = builder.getAppId();            //应用的ID
                request.partnerId = builder.getPartnerId();     //商户号
                request.prepayId = builder.getPrepayId();       //预支付交易会话
                request.packageValue = "Sign=WXPay";            //拓展字段
                request.nonceStr = builder.getNonceStr();       //随机字符串
                request.timeStamp = builder.getTimeStamp();     //时间戳
                request.sign = builder.getSign();               //签名
                Log.e("chx", "run: " + request.appId + request.nonceStr + request.sign);
                //endReq是第三方app主动发送消息给微信，发送完成之后会切回到第三方app界面。
                iwxapi.sendReq(request);//发送调起微信的请求,这样就交给微信了，剩下的就是用户输入密码等，我们只要等着拿结果
            }
        };
